...
async def session_select_group(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Fan nomini yozing:", reply_markup=cancel_keyboard())
...
async def start_add_direction(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text("➕ <b>Yangi yo'nalish qo'shish</b>\n\nYo'nalish nomini kiriting:")
    await callback.message.answer("Yo'nalish nomini yozing:", reply_markup=cancel_keyboard())
...
async def add_group_direction(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"📚 Yo'nalish: <b>{direction}</b>\n\nGuruh nomini kiriting:")
    await callback.message.answer("Guruh nomini yozing (masalan: 101):", reply_markup=cancel_keyboard())
